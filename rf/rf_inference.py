...
model_save_path = os.path.join(parent_dir, 'models', 'RF_Models', 'rf_model_40pct.pkl')
encoder_save_path = os.path.join(parent_dir, 'models', 'RF_Models', 'label_encoder_40pct.pkl')

# Load the trained model and label encoder
rf_model = joblib.load(model_save_path)
label_encoder = joblib.load(encoder_save_path)

# Input fields corresponding to the features used in training (modified)
input_fields = [
...
def generate_report():
    ...
    predicted_stage = rf_model.predict(user_input)
    
    # Decode the predicted stage
    predicted_stage_label = label_encoder.inverse_transform(predicted_stage)
