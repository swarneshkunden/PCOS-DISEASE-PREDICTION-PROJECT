...
model_save_path = os.path.join(parent_dir, 'models', 'BILSTM_Models', 'bilstm_model_40pct.h5')
encoder_save_path = os.path.join(parent_dir, 'models', 'BILSTM_Models', 'label_encoder_40pct.pkl')

# Load the trained BiLSTM model and label encoder
bilstm_model = load_model(model_save_path)
label_encoder = joblib.load(encoder_save_path)

# Updated list of input fields (excluding 'PCOS Stage' field)
input_fields = [
...
def generate_report():
    ...
    for field in input_fields:  # Iterate through all input fields
        try:
    ...
    # Preprocess the input data for BiLSTM model
    user_input = pad_sequences(user_input, maxlen=30, padding='post')  # Adjust the maxlen as per your training data
    
    # Predict the PCOS stage using the trained model
    predicted_stage = bilstm_model.predict(user_input)
...
# Split the input fields into 5-5-4 parts
